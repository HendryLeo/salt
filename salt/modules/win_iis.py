...
def modify_cert_binding(name, site, hostheader="", ipaddress="*", port=443, sslflags=0):
    """
    Modify certificate to an IIS Web Binding.
    .. note::
        The web binding that the certificate is being assigned to must already
        exist.
    Args:
    ...
    CLI Example:
    .. code-block:: bash
        salt '*' win_iis.modify_cert_binding name='AAA000' site='site0' hostheader='example.com' ipaddress='*' port='443'
    """
    name = str(name).upper()
    binding_info = _get_binding_info(hostheader, ipaddress, port)
    ...
    for current_binding in current_bindings:
        if binding_info == current_binding:
            current_name = current_bindings[current_binding]["certificatehash"]
            if current_name:
                remove_success = remove_cert_binding(current_name, site, hostheader, ipaddress, port)
                if remove_success:
                    log.debug("Removed current certificate thumbprint: %s", current_name)
                    current_name = None
